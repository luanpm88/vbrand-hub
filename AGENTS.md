...
## Codex là owner của docs/ và bots/

`docs/**` và `bots/**` là **source of truth** của project — Codex chịu trách nhiệm giữ chúng đúng và đầy đủ. Đây không phải là "tài liệu phụ", đây là **bộ nhớ dài hạn** của AI cho project này.
...
- **Mỗi lần fix bug có root cause đáng nhớ** → update `## Lessons Learned` trong AGENTS.md hoặc thêm note vào doc liên quan.
...
| Fix bug có lesson | `AGENTS.md ## Lessons Learned` + comment ở chỗ fix |
...
| Hoàn thành 1 phase E2E | Mark ☑ trong `docs/E2E_TEST_PLAN.md` + ghi lessons vào AGENTS.md nếu có |
| Tìm ra credential/setup mới cho local/staging/prod | `AGENTS.md ## Server` hoặc relevant env section |
...
- **App updates (after 2026-06-03 cutover):** vbrand runs on acelle mainline + brand plugin. Refer user to `acelle/AGENTS.md` for mainline app updates; `acelle_brand/AGENTS.md` for plugin updates. Deploy app: build acelle release alongside, set `.env` (`DB_DATABASE=brand`, `APP_URL=https://app.sgconnect.vn`, keep legacy `APP_KEY`/mail), `composer install --no-dev`, `migrate:fresh` + seed `DatabaseInit` + `TemplateSeeder`, then `mv app app-legacy && ln -sfn app-new app` (no sudo needed — nginx `SCRIPT_FILENAME=$realpath_root` + opcache revalidate_freq=2). Rollback: `rm /home/vbrand/app && mv /home/vbrand/app-legacy /home/vbrand/app`.
...
- Production sellers go straight from `delivering` → `completed` (the 4-step workflow). Per Codex rule "if 2 docs disagree → find the right one → fix the wrong one", the user guides are wrong; the app is right. **Fixed by updating both user guides** to the 4-step workflow with status names in parens.
...
User có thể nói ngắn — Codex phải tự hiểu và chạy đúng bot:
...
9. Update `## Lessons Learned` trong AGENTS.md nếu bug có root cause đáng nhớ (vd: WP theme schema null, missing route, broken middleware)
